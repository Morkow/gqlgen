...
func TestChatSubscriptions(t *testing.T) {
	...
	go func() {
		...
		err := c.Post(`mutation { 
				a:post(text:"Hello!", roomName:"#gophers", username:"Morkow") { id } 
				b:post(text:"Whats up?", roomName:"#gophers", username:"Morkow") { id } 
			}`, &resp)
	...
	require.Equal(t, "Morkow", msg.resp.MessageAdded.CreatedBy)
